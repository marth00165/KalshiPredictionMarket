...
## Profitability Roadmap (Priority Order)
Focus less on winner prediction tweaks and more on mispricing capture after fees/slippage.

1. Build no-vig sportsbook consensus ingestion (multi-book, normalized mapping, timestamped).
2. Add robust de-vig calculations and store per-book fair probs + consensus fair prob.
3. Add Kalshi-vs-consensus divergence signal (edge, z-score, persistence window).
4. Gate entries on net edge only (after taker fee + slippage + spread/impact).
5. Replace fixed slippage with orderbook-depth-aware slippage model.
6. Add execution policy (maker preference, chase limits, cancel/replace, tipoff cutoffs).
7. Tighten signal quality filters (liquidity, volume, persistence, news-whipsaw guardrails).
8. Add portfolio risk constraints (per-game cap, correlation cap, drawdown breaker).
9. Build true replay/backtest with historical Kalshi prices + settlement labels.
10. Track KPI dashboard: CLV, net EV, fee drag, fill quality, ROI, drawdown.
11. Keep injury adjustment deterministic and lower weight versus market-pricing signals.
12. Add stronger observability: per-trade attribution, model/config version snapshot, alerts.
